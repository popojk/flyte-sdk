"""An OpenAI Agents agent, instrumented for Grafana Agent Observability.

The agent code is unchanged from the plain flyteplugins-agents-openai version. Calling init
at module scope is the whole integration: the adapter offers its run options to an
instrumentor on the way to Runner.run, which is how a RunHooks reaches a call the adapter
owns rather than you.

    pip install "flyteplugins-agento11y[openai]"
    export AGENTO11Y_ENDPOINT=... AGENTO11Y_AUTH_TOKEN=...
    python openai_agent.py            # on the cluster in your flyte config
    python openai_agent.py --local    # in-process, no cluster needed
"""
...
init(service_name="support-agent", exporter=_console_or_otlp(), disable_batch=True)
...
async def support_agent(question: str = "Where is order A-1001?") -> str:
...
if __name__ == "__main__":
    ...
    print(runner.run(support_agent, question="Where is order A-1001?").url)
