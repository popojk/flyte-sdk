"""A Pydantic AI agent, instrumented for Grafana Agent Observability.

The agent code is unchanged from the plain flyteplugins-agents-pydantic_ai version. Calling init at
module scope is the whole integration: the adapter offers its run capabilities to an instrumentor on the
way past, which is how an agento11y capability reaches a call the adapter owns rather than you.

    pip install "flyteplugins-agento11y[pydantic-ai]"
    python pydantic_ai_agent.py            # on the cluster in your flyte config
    python pydantic_ai_agent.py --local    # in-process, no cluster needed
"""
...
init(service_name="pydantic-support", exporter=_console_or_otlp(), disable_batch=True)

image = (
    flyte.Image.from_debian_base()
    # The agento11y framework integration is a PyPI package, not one of ours.
    .with_pip_packages("agento11y-pydantic-ai")
    ...
    .with_local_v2_plugins(
        [
            ...
            "flyteplugins-agents-pydantic_ai",
            "flyteplugins-otel",
...
async def pydantic_ai_agent(question: str = "Where is order A-1001?") -> str:
...
if __name__ == "__main__":
    ...
    print(runner.run(pydantic_ai_agent, question="Where is order A-1001?").url)
